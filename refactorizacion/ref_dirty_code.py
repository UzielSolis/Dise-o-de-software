...
class Zombie:
    ...
    def tiempo_sin_alimento(self):
        ''' Regresa el tiempo que ha pasado desde que el zombie comió su último cerebro '''
    ...
    @property
    def estado_de_animo_bueno(self):
        ''' Regresa el estado de ánimo de nuestro zombie basado en el tiempo
            que ha pasado sin comer '''
        if datetime.now() - self.__ultimo_alimento > timedelta(seconds=10):
            return 'agresivo'
        elif datetime.now() - self.__ultimo_alimento > timedelta(seconds=20):
            return 'enfadado'
        elif datetime.now() - self.__ultimo_alimento > timedelta(seconds=5):
            return 'normal'
        else:
            return 'contento'
    ...
    def peligrosidad(self):
        ...
        aggresive_zombie = self.__tipo == TipoZombie.ZOMBIEINSTEIN or self.__tipo == TipoZombie.ZOMBIEPULTA
        estado_de_animo_bueno = self.estado_de_animo_bueno == 'contento' or self.estado_de_animo_bueno == 'normal'

        if not aggresive_zombie and estado_de_animo_bueno and self.__cerebros_devorados >= 1:
            return 'baja'
        elif not aggresive_zombie and not estado_de_animo_bueno and self.__cerebros_devorados >= 1:
            return 'media'
        else:
            return 'alta'
...
if __name__ == "__main__":
    ...
    # Hacemos uso de la propiedad estado_de_animo_bueno que a su vez reusa el código
    # dentro de la propiedad tiempo_sin_alimento
    print(f'-I- Nuestro zombie está {zombie_bailarin.estado_de_animo_bueno}')
